...
class Document(object):
    ...
    def __init__(self, ruc, data, client):
        _logger.debug('Inicia proceso del documento : ')
        self._ruc = ruc
    ...
    def generate_document_name(self):
         _logger.debug('Genera nombre del documento XML : ')

    def validate(self):
        _logger.debug('Valida XML : ')

    def render(self):
        _logger.debug('Renderiza XML con valores del Json : ')
        template = env.get_template(self.template_name)
        self._xml = template.render(**self._data)
        _logger.debug('XML Renderizado : ' + self._xml)
    ...
    def writetofile(self, filename, filecontent):
        _logger.debug('Escribe en archivo : ')
        self.in_memory_zip.writestr(filename,filecontent)

    def prepare_zip(self):
        _logger.debug('Prepara ZIP : ' + self._xml)
        _logger.debug('Nombre del archivo XML a enviar : ' + self._document_name)
    ...
    def send(self):
        _logger.debug('Enviando XML a servicio de Sunat : ')
    ...
    def process_response(self):
        _logger.debug('Procesamiento completo de XML : ')
